...
std::ostream& operator<<(std::ostream &out, VMValue const& a) {
    ...
    switch(a.vartype) {
        case VMValueType::UINT:   out << 'u' << std::dec << (int)a.size*8 << '(' << std::to_string(a.value.uInt); break;
        case VMValueType::SINT:   out << 's' << std::dec << (int)a.size*8 << '(' << std::to_string(a.value.sInt); break;
